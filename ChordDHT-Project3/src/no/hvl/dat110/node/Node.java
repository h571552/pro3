...
public class Node extends UnicastRemoteObject implements ChordNodeInterface {
	...
	private boolean multicastMessage(Message message) throws AccessException, RemoteException {
		...
		for(int i = 0; i < quorum; i++) {
			...
			n = n.getSuccessor();
			queueACK.add(n.onMessageReceived(message));
	...
	public void onReceivedUpdateOperation(Message message) throws RemoteException {
		...
		if(message.getOptype() == OperationType.WRITE) {
			...
			new Operations(this, message, activenodesforfile).performOperation();
			releaseLocks();
		...
		if(message.getOptype() == OperationType.WRITE) {
			...
			for(Message replica : activenodesforfile) {
				...
				try {
					...
					ChordNodeInterface node = (ChordNodeInterface) registry.lookup(replica.getNodeID().toString());
					node.onReceivedUpdateOperation(replica);
